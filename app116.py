...
excel_data_df = pandas.read_excel('Book1.xlsx', sheet_name='Sheet116')
...
layout = html.Div([
    html.H3(('Bedroom (Node 116)'),style={'textAlign': 'center'}),
    html.Div([
        dcc.Dropdown(
            id='xaxis116',
...
html.A(html.Button('Go to Living Room (Node 27)', id='button-1', className='mr-1',style={'backgroundColor':'#C2F9EE'}),href='/apps/app27'),
# dcc.Link('Go to Node 105', href='/nodes/node31')
])

@app.callback(Output('graph7', 'figure'),
              [Input('xaxis116', 'value')])

## Main Plotting Function

def update_graph(xaxis116_name):
    return {
           'data': [go.Scatter(
            x=excel_data_df['Date'],
            y=excel_data_df[xaxis116_name],
        ...
        'layout': go.Layout(
            ...
            yaxis={'title': xaxis116_name.title()},
            title="Line Plot",
...
def update_graph(xaxis116_name):
    return {
        ...
        'layout': go.Layout(
            xaxis={'title': xaxis116_name.title()},
