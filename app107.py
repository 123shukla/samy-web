...
excel_data_df = pandas.read_excel('Book1.xlsx', sheet_name='Sheet107')
...
layout = html.Div([
    html.H3(('Bedroom-1 (Node 107)'),style={'textAlign': 'center'}),
    html.Div([
        dcc.Dropdown(
            id='xaxis107',
...
html.Div([  # this Div contains our output graph
        dcc.Graph(id='graph30')
        ], style={'width': '100%'}),
    html.Div([  # this Div contains our output graph
        dcc.Graph(id='graph31')
        ], style={'width': '100%'}),

# html.Div(id='node-31-display-value'),
html.A(html.Button('Go to Bedroom-2 (Node 108)', id='button-1', className='mr-1',style={'backgroundColor':'#C2F9EE'}),href='/apps/app108'),
# dcc.Link('Go to Node 105', href='/nodes/node31')
])

@app.callback(Output('graph30', 'figure'),
              [Input('xaxis107', 'value')])

## Main Plotting Function

def update_graph(xaxis107_name):
    return {
           'data': [go.Scatter(
            x=excel_data_df['Date'],
            y=excel_data_df[xaxis107_name],
        ...
        'layout': go.Layout(
            ...
            yaxis={'title': xaxis107_name.title()},
            title="Line Plot",
...
def update_graph(xaxis107_name):
    return {
        ...
        'layout': go.Layout(
            xaxis={'title': xaxis107_name.title()},
