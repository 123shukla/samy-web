...
excel_data_df = pandas.read_excel('Book1.xlsx', sheet_name='Sheet109')
...
layout = html.Div([
    html.H3(('Bedroom (Node 109)'),style={'textAlign': 'center'}),
    html.Div([
        dcc.Dropdown(
            id='xaxis109',
...
html.A(html.Button('Go to Living Room (Node 186)', id='button-1', className='mr-1',style={'backgroundColor':'#C2F9EE'}),href='/apps/app186'),
# dcc.Link('Go to Node 105', href='/nodes/node31')
])

@app.callback(Output('graph16', 'figure'),
              [Input('xaxis109', 'value')])

## Main Plotting Function

def update_graph(xaxis109_name):
    return {
           'data': [go.Scatter(
            x=excel_data_df['Date'],
            y=excel_data_df[xaxis109_name],
        ...
        'layout': go.Layout(
            ...
            yaxis={'title': xaxis109_name.title()},
            title="Line Plot",
...
@app.callback(Output('graph17', 'figure'),
              [Input('xaxis109', 'value')])
...
def update_graph(xaxis109_name):
    return {
        ...
        'layout': go.Layout(
            xaxis={'title': xaxis109_name.title()},
