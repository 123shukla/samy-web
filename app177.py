...
excel_data_df = pandas.read_excel('Book1.xlsx', sheet_name='Sheet177')
...
layout = html.Div([
    html.H3(('Living Room (Node Sheet177)'),style={'textAlign': 'center'}),
    html.Div([
        dcc.Dropdown(
            id='xaxis177',
...
html.Div([  # this Div contains our output graph
        dcc.Graph(id='graph177')
        ], style={'width': '100%'}),
    html.Div([  # this Div contains our output graph
        dcc.Graph(id='graph178')
        ], style={'width': '100%'}),
...
@app.callback(Output('graph177', 'figure'),
              [Input('xaxis177', 'value')])

## Main Plotting Function

def update_graph(xaxis177_name):
    return {
           'data': [go.Scatter(
            x=excel_data_df['Date'],
            y=excel_data_df[xaxis177_name],
        ...
        'layout': go.Layout(
            ...
            yaxis={'title': xaxis177_name.title()},
            title="Line Plot",
...
@app.callback(Output('graph178', 'figure'),
              [Input('xaxis177', 'value')])
...
def update_graph(xaxis177_name):
    return {
        ...
        'layout': go.Layout(
            xaxis={'title': xaxis177_name.title()},
